            ...
            <div class="portlet light portlet-datatable bordered">
                <div class="layout-navbar navbar-detached shadow-sm p-3 bg-white rounded row container-xxl flex-grow-1 container-p-y">
                    ...
                    <div class="actions text-right col-6">
                        <a href="{{ route('ad.productcategory.create') }}" class="btn btn-circle btn-sm btn-primary"> <i class="fa fa-plus"></i> Tạo mới</a>
                    </div>
                </div>
                        ...
                        <table class="table table-bordered table-hover" id="admin-table">
                            ...
                            @foreach ($categories as $key=>$category)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn btn-circle btn-sm btn-warning" href="{{ route('ad.productcategory.edit', $category) }}"><i class='bx bx-edit' ></i></a>
                                        <form action="{{ route('ad.productcategory.destroy', $category) }}" id="delete-course-item-form" style="display: inline-block" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa?');">
                                            @csrf
                                            @method('DELETE')
