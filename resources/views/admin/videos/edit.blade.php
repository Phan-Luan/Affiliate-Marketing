...
@section('page_title', 'Cập nhật Video')
        ...
        <div class="col-md-12">
            <div class="portlet light portlet-datatable bordered">
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <form action="{{ route('ad.video.update', $video) }}" method="POST" @submit.prevent="onSubmit" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-md-8">
                                <div class="form-group" :class="[errors.has('key') ? 'has-error' : '']">
                                    ...
                                    <input type="text" id="name" class="form-control" name="name" required value="{{ old('name', $video->name) }}">
                                </div>
                                <div class="form-group" :class="[errors.has('key') ? 'has-error' : '']">
                                    <label for="position">Vị trí <span class="required">*</span></label>
                                    <input type="number" id="position" class="form-control" name="position" required value="{{ old('position', $video->position) }}">
                                </div>
                                <div class="form-group" :class="[errors.has('value') ? 'has-error' : '']">
                                    <label for="iframe">Iframe <span class="required">*</span></label>
                                    <textarea class="form-control" name="iframe" id="iframe" cols="30" rows="10" required>{{ old('iframe', $video->iframe) }}</textarea>
                                </div>
                                <div class="form-group" :class="[errors.has('value') ? 'has-error' : '']">
                                    <label for="description">Mô tả <span class="required">*</span></label>
                                    <textarea class="form-control" name="description" id="description" cols="30" rows="6" required>{{ old('description', $video->description) }}</textarea>
                                </div>
                            </div>
                        </div>
